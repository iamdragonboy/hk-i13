...
@bot.tree.command(name="create-vps")
...
class ManageView(ui.View):
    ...
    async def restart(self, interaction: Interaction, button: ui.Button):
        ...
        await interaction.response.send_message(f"🔁 VPS `{self.container}` restarted.", ephemeral=True)

    @ui.button(label="Status", style=discord.ButtonStyle.secondary)
    async def status(self, interaction: Interaction, button: ui.Button):
        status = subprocess.getoutput(f"docker inspect -f '{{{{.State.Running}}}}' {self.container}").strip()
        result = "🟢 Online" if status == "true" else "🔴 Offline"
        await interaction.response.send_message(f"📡 VPS `{self.container}` Status: {result}", ephemeral=True)

    @ui.button(label="Run CMD", style=discord.ButtonStyle.blurple)
    async def runcmd(self, interaction: Interaction, button: ui.Button):
        await interaction.response.send_modal(CMDModal(self.container))

class CMDModal(ui.Modal, title="Run Command in VPS"):
    command = ui.TextInput(label="Enter Command", placeholder="e.g. apt update", style=discord.TextStyle.paragraph)
    ...
    async def on_submit(self, interaction: Interaction):
        cmd = f"docker exec {self.container} bash -c \"{self.command.value}\""
        try:
...
class VPSManager(commands.Cog):
    ...
    @app_commands.command(name="manage", description="🧰 Manage your VPS")
    async def manage(self, interaction: Interaction):
        ...
        with open("database.txt") as f:
            lines = f.readlines()
        user_vps = [line.strip() for line in lines if line.strip().endswith(f"|{user_id}")]

        if not user_vps:
            await interaction.response.send_message("❌ You don’t own any VPS.", ephemeral=True)
            return

        container = user_vps[0].split("|")[0]
        embed = Embed(title="🛠️ VPS Manager", description=f"Container: `{container}`", color=discord.Color.green())
        await interaction.response.send_message(embed=embed, view=ManageView(container), ephemeral=True)
...
class NodeStatusView(ui.View):
    ...
    async def refresh(self, interaction: Interaction, button: ui.Button):
        if not self.is_owner(interaction):
            await interaction.response.send_message("❌ Only you can use this button.", ephemeral=True)
            return
        await self.send_updated_embed(interaction)

    @ui.button(label="▶️ Start All", style=discord.ButtonStyle.success)
    async def start_all(self, interaction: Interaction, button: ui.Button):
        if not self.is_owner(interaction):
            await interaction.response.send_message("❌ Only you can use this button.", ephemeral=True)
            return
        os.system("docker start $(docker ps -a -q)")
        await interaction.response.send_message("🟢 All containers started.", ephemeral=True)

    @ui.button(label="⛔ Stop All", style=discord.ButtonStyle.danger)
    async def stop_all(self, interaction: Interaction, button: ui.Button):
        if not self.is_owner(interaction):
            await interaction.response.send_message("❌ Only you can use this button.", ephemeral=True)
            return
        os.system("docker stop $(docker ps -q)")
        await interaction.response.send_message("🔴 All containers stopped.", ephemeral=True)

    @ui.button(label="🔄 Restart All", style=discord.ButtonStyle.secondary)
    async def restart_all(self, interaction: Interaction, button: ui.Button):
        if not self.is_owner(interaction):
            await interaction.response.send_message("❌ Only you can use this button.", ephemeral=True)
            return
        os.system("docker restart $(docker ps -q)")
        await interaction.response.send_message("🔁 All containers restarted.", ephemeral=True)

    @ui.button(label="🔐 SSH Info", style=discord.ButtonStyle.blurple)
    async def ssh_info(self, interaction: Interaction, button: ui.Button):
        if not self.is_owner(interaction):
            await interaction.response.send_message("❌ Only you can use this button.", ephemeral=True)
            return
        ssh_details = "Host: your-vps-ip\nPort: 22\nUsername: root\nPassword: yourpass"
        await interaction.response.send_message(f"```{ssh_details}```", ephemeral=True)

    @ui.button(label="📡 Status", style=discord.ButtonStyle.gray)
    async def node_status(self, interaction: Interaction, button: ui.Button):
        if not self.is_owner(interaction):
            await interaction.response.send_message("❌ Only you can use this button.", ephemeral=True)
            return
        await self.send_updated_embed(interaction)

    async def send_updated_embed(self, interaction):
        cpu = "Ryzen 9 5950X"
        ram = f"{round(psutil.virtual_memory().used / (1024 ** 3), 2)} GB / {round(psutil.virtual_memory().total / (1024 ** 3), 2)} GB"
        disk = f"{round(psutil.disk_usage('/').used / (1024 ** 3), 2)} GB / {round(psutil.disk_usage('/').total / (1024 ** 3), 2)} GB"
        uptime_seconds = float(os.popen('cat /proc/uptime').read().split()[0])
        uptime_hours = round(uptime_seconds / 3600, 2)

        embed = Embed(title="🛰️ Node Info", color=discord.Color.green())
        embed.add_field(name="CPU", value=cpu, inline=True)
        embed.add_field(name="RAM Usage", value=ram, inline=True)
        embed.add_field(name="Disk Usage", value=disk, inline=True)
...
class NodeStats(commands.Cog):
    ...
    @app_commands.command(name="VpsInfo", description="📊 Show real-time VPS node status")
    async def nodes(self, interaction: Interaction):
        ...
        await interaction.response.send_message(embed=embed, view=NodeStatusView(interaction.user.id), ephemeral=True)
